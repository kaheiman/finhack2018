...
app.directive('image', ['$q',
	function ($q) {
		...
		var jicCompress = function (sourceImgObj, options) {
			...
			if (outputFormat !== undefined && outputFormat === 'image/png') {
				mimeType = 'image/png';
			}
		...
		return {
			...
			link: function postLink(scope, element, attrs, CommonCtrl) {
				...
				var applyScope = function (imageResult, gif) {
					scope.onChange({
						img: imageResult,
						gif_type: gif
					});
				...
				element.bind('change', function (evt) {
					...
					for (var i = 0; i < files.length; i++) {
						...
						if (files[i].type != "image/gif") { //resize image
							doResizing(imageResult, function (imageResult) {
								applyScope(imageResult, false);
							});
						} else { //no resizing
							applyScope(files[i], true);
						}
					}
				});
			}
